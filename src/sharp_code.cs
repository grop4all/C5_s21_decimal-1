...
class Decimal_testing {
  static void Main() {
    ...
    string[] Ops = {"ADD" , "SUB", "MUL", "DIV", "MOD", "GREAT", "LESS", "EQUAL","NOTEQUAL", 
                      "GREATEQ", "LESSEQ", "FLOOR", "ROUND", "TRUNCATE", "NEGATE"};
    string[] Ops1 = {"add" , "sub", "mul", "div", "mod", "is_greater", "is_less", "is_equal", 
                     "is_not_equal", "is_greater_or_equal", "is_less_or_equal", "floor", "round", "truncate", "negate"};
    ...
    if (Noper==11) num_result = Decimal.Floor(number1);
    if (Noper==12) num_result = Decimal.Round(number1);
    if (Noper==13) num_result = Decimal.Truncate(number1);
    if (Noper==14) num_result = Decimal.Negate(number1);
    ...
    if (Noper==6) func_res = number1 < number2;
    if (Noper==7) func_res = number1 == number2;
    if (Noper==8) func_res = number1 != number2;
    if (Noper==9) func_res = number1 >= number2;
    if (Noper==10) func_res = number1 <= number2;
    ...
    if (Noper >10) {
        Console.WriteLine($"s21_decimal num1 = new_decimal({(uint)num1[0]}, {(uint)num1[1]}, {(uint)num1[2]}, {(uint)num1[3]}, {(uint)num1[4]});");
        Console.WriteLine($"s21_decimal result;");
        Console.WriteLine($"ck_assert_uint_eq(s21_{Ops1[Noper]}(num1, &result), 0);");
        Console.WriteLine($"ck_assert_uint_eq(result.bits[0], {(uint)bits2[0]});");
        Console.WriteLine($"ck_assert_uint_eq(result.bits[1], {(uint)bits2[1]});");
        Console.WriteLine($"ck_assert_uint_eq(result.bits[2], {(uint)bits2[2]});");
        Console.WriteLine($"ck_assert_uint_eq(result.bits[3], {(uint)bits2[3]});\n");
